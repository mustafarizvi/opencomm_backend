...
import org.hsc.sip.ua.config.data.Constants;
...
public class UserAgent extends CallListenerAdapter {
	...
	protected void launchMediaApplication() {
		 ...
		 * LogLevel.HIGH); return; } SessionDescriptor local_sdp = new
		 * SessionDescriptor(call .getLocalSessionDescriptor()); String
		 * local_media_address = (new Parser(local_sdp.getConnection()
		 * .toString())).skipString().skipString().getString(); int
		 * local_audio_port = 0; int local_video_port = 0;
		 * 
		 * // parse local sdp for (Enumeration<MediaDescriptor> e =
		 * local_sdp.getMediaDescriptors() .elements(); e.hasMoreElements();) {
		 * MediaField media = e.nextElement().getMedia(); if
		 * (media.getMedia().equals("audio")) local_audio_port =
		 * media.getPort(); if (media.getMedia().equals("video"))
		 * local_video_port = media.getPort(); } // parse remote sdp
		 * SessionDescriptor remote_sdp = new SessionDescriptor(call
		 * .getRemoteSessionDescriptor()); String remote_media_address = (new
		 * Parser(remote_sdp.getConnection()
		 * .toString())).skipString().skipString().getString(); int
		 * remote_audio_port = 0; int remote_video_port = 0; for
		 * (Enumeration<MediaDescriptor> e = remote_sdp.getMediaDescriptors()
		 * .elements(); e.hasMoreElements();) { MediaField media =
		 * e.nextElement().getMedia(); if (media.getMedia().equals("audio"))
		 * remote_audio_port = media.getPort(); if
		 * (media.getMedia().equals("video")) remote_video_port =
		 * media.getPort(); }
		 * 
		 * // select the media direction (send_only, recv_ony, fullduplex) int
		 * dir = 0; if (user_profile.recv_only) dir = -1; else if
		 * (user_profile.send_only) dir = 1;
		 * 
		 * if (user_profile.audio && local_audio_port != 0 && remote_audio_port
		 * != 0) { // create an audio_app and start // it
		 * 
		 * if (audio_app == null) { // for testing.. String audio_in = null; if
		 * (user_profile.send_tone) { audio_in = JAudioLauncher.TONE; } else if
		 * (user_profile.send_file != null) { audio_in = user_profile.send_file;
		 * } String audio_out = null; if (user_profile.recv_file != null) {
		 ...
		 * user_profile.audio_frame_size, log); } audio_app.startMedia(); }
		 */
	}
